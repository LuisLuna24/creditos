<div>
        ...
        <x-slot name="rows">
            @forelse ($collection as $index => $item)
                <x-table.tr>
                    ...
                    <x-table.td-buttons>
                        <x-table.button-table tipo="view" wire:click="view({{ $item->taller_id }})" />
                        <x-table.button-table tipo="edit" wire:click="edit({{ $item->taller_id }})" />
                        <x-table.button-table tipo="delete" wire:click="delete({{ $item->taller_id }})" />
                    </x-table.td-buttons>
                </x-table.tr>
            @empty
                <x-table.empty-state cols="6" message="No hay usuarios disponibles" />
            @endforelse
    ...
    <x-dialog-modal wire:model="deleteModal">
        ...
        <x-slot name="content">
            <p class="text-center">¿Desea eliminar este taller?</p>
    ...
    <x-dialog-modal wire:model="estatusModal">
        ...
        <x-slot name="content">
            <p class="text-center">¿Desea cambiar el estatus de este taller?</p>
